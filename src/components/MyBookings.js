...
class MyFlights extends Component {
  ...
  createAndDownloadPdf = (booking) => {
    this.setState({ loadingCheckIn: true, showPdf: true ,selectedBooking: booking}); // Show the PDF viewer
    setTimeout(() => {
      this.setState({ loadingCheckIn: false });
    }, 1000);

    // axios
    //   .post("https://bms-backend-swart.vercel.app/create-pdf", booking, {
    ...
    return (
      <div>
        ...
        {this.state.showPdf && (
          ...
          >
            <Modal.Header closeButton>
              <Modal.Title>Your Boarding Pass</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              
                <Pdf  booking={this.state.selectedBooking}/>
