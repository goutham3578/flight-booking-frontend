...
class BookFlight extends Component {
  ...
  async loadPassengers() {
    console.log("loadPassengers called", this.props.user);
    if (this.props.user) {
      console.log("Fetching user details for userId:", this.props.user.userId);
  ...
  async onSubmit(formData) {
    ...
    if (res) {
      await this.props.addUserDetails(this.props.user.userId, formData);
      this.loadPassengers();
      this.state.showForm = false;
    }
  }
  render() {
    const { handleSubmit,flight,userDetails } = this.props;
 // Show loading state until flight data is fetched
 if (!flight || Object.keys(flight).length === 0) {
  return (
    <Card>
      <Card.Body>
        <Card.Text>No flight selected to book</Card.Text>
...
function mapStateToProps(state) {
  console.log(state.flight.flight)
  return {
